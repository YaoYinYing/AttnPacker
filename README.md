...
Inference with AttnPacker is outlined in `AttenPacker/examples/inference.ipynb`. This includes examples for sequence design, side-chain post processing 
per-residue confidence prediction and more. A notbook with examples specific to sampling is available at `AttenPacker/examples/sampling.ipynb`. Additional examples are outlined below.
...
(py38)[mmcpartlon@raptorx11 AttnPacker]$ python AttenPacker/examples/post_process.py ./AttenPacker/examples/pdbs/T1057-predicted.pdb --steric_tol_allowance 0 --steric_tol_frac 0.95 --max_optim_iters 200 --device cuda:0
...
Saving to: ./AttenPacker/examples/pdbs/post-processed-T1057-predicted.pdb
...
```python
from AttenPacker.assessment.sidechain import assess_sidechains, summarize
import pprint
